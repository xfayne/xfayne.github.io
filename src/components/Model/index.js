...
const Model = ({
  ...
  useEffect(() => {
    ...
    const modelConfigPromises = models.map(async (model, index) => {
      const { texture, position, url, rotate, scale } = model;
      ...
      if(rotate){
        gltf.scene.rotation.set(...rotate);
      }
      ...
      gltf.scene.traverse(async node => {
        ...
        if (node.name === 'pasted__polySurface18_pasted__blinn26_0') {
          //watch screen
  ...
  useEffect(() => {
    ...
    const loadScene = async () => {
      ...
      const handleModelLoad = loadedModels.map(async model => {
        ...
        model.loadFullResTexture && await model.loadFullResTexture();
